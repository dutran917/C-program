...
void checkEqualvalue(int values,int values2)
{
  if (values < values2)
    error(ERR_EXPESSION_MORE_THAN, currentToken->lineNo, currentToken->colNo);
  else
    if(values > values2)
      error(ERR_ASSIGN_MORE_THAN, currentToken->lineNo, currentToken->colNo);
}
