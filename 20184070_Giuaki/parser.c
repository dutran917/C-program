...
void compileAssignSt(void) {
  ...
  Type **assvalue = (Type**)calloc(10,sizeof(Type*));
  Type **expvalue = (Type**)calloc(10,sizeof(Type*));
  int values = 0;
  int values2 = 0;
  assvalue[values++] = compileLValue();
  ...
  eat(SB_ASSIGN);
  expvalue[values2++] = compileExpression();
  while(lookAhead->tokenType == SB_COMMA)
  {
  	eat(SB_COMMA);
  	expvalue[values2] = compileExpression();
  ...
  checkEqualvalue(values,values2);

  for(int i=0;i<values;i++)
  {
  	// printf("-------\n");
  	checkTypeEquality(assvalue[i],expvalue[i]);
  }
}
...
Type* compileExpression2(void) {
  ...
  {
    if((type->typeClass == TP_INT && type_->typeClass == TP_DOUBLE) || (type->typeClass == TP_DOUBLE && type_->typeClass == TP_INT))
      {
